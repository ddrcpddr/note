...
- Android 本地 SQLite schema 已补齐 categories、members、	ags 表；此前原生 SQLite 主要覆盖 notes、attachments 和 sync_queue。
...
- 浏览器 IndexedDB 路径保持可用，并补齐 ttachments 快照写入 / 返回，避免浏览器和 Android 路径行为分叉。
