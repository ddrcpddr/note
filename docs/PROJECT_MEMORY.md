...
- 导入页已收敛步骤条和文件卡：步骤圆点 36px，文件卡标题 16px，待选择状态显示 等待选择 .nsx 文件，移除 truncate，底部操作栏高度缩小。
- 验证结果：
npm.cmd run check 通过，integrityCheck=ok、
oteCount=114；
npm.cmd run test 通过 11 suites / 40 tests；
npm.cmd run build 通过；docker compose up -d --build 后
npm.cmd run smoke -- --base-url http://127.0.0.1:3300 通过。
- Playwright 390px / 430px 验证：设置页和导入页均无页面级横向溢出；设置页 上次备份 文案高度 18px，不再竖排；导入页 .nsx 标题完整显示且不再使用 truncate。
...
-
npm.cmd run smoke 增加真实写入检查：创建新笔记、读取新笔记详情、上传 .nsx fixture、dry-run、commit、确认导入图片附件进入富文本。
...
- Fixed by sanitizing all IndexedDB writes through toIndexedDbSafeValue() in src/client/offlineStore.js; it strips functions, symbols, undefined values, and circular references before put().
...
## 2026-07-06 Gate 13：恢复联网同步批处理行为测试

继续推进离线 APK 的真实可用性。本轮把“恢复联网后处理待同步队列”的核心循环从 `src/client/main.jsx` 抽到：
...
- 离线 create mutation 会发到 `/api/notes`，方法为 `POST`。
- 离线 update mutation 会发到 `/api/notes/:id`，方法为 `PATCH`。
- 批处理同步时，如果第一条成功，会写回本地 synced note、删除 local-* 临时记录、删除成功 mutation。
- 如果后续 mutation 失败，会标记 failed 并停止本轮批处理，避免继续乱同步后续记录。
- UI 仍通过同一回调更新列表和当前选中记录。

验证：

- `node --test tests/offline-sync-behavior.test.js tests/offline-store-behavior.test.js tests/offline-store-static.test.js tests/frontend-ui.test.js`：通过，30 tests。
- `npm.cmd run check`：通过，SQLite `integrityCheck=ok`。
- `npm.cmd run test`：通过，16 suites / 86 tests。
- `npm.cmd run build`：通过。
- `npm.cmd run android:build`：通过，APK 已重新生成。
- `npm.cmd run android:verify`：通过，APK 内部离线 bundle 包含新前端 JS。
- `npm.cmd run android:delivery-check`：通过，内部再次覆盖 check/test/build/android build/android verify/HTTP smoke。

经验：

- 离线恢复联网不能只靠源码字符串检查；至少要有行为测试证明成功项会清理、失败项会保留并停止后续批处理。
- 这仍不是完整真机验收。最终仍需在 vivo X300 Pro 和 Huawei P30 Pro / HarmonyOS 上执行 APK 离线使用、恢复联网同步和日志检查。

## 2026-07-06 Gate 14：离线 APK 内可重新修改服务器地址

用户再次强调：这是家庭自用工具，不要按大项目流程开发，最终要交付真正测过、关键 bug 尽量少、可离线使用的 Android App。本轮继续按小 Gate 推进。

本轮修复一个恢复联网路径缺口：此前 Android 原生壳只有启动设置页 / 连接失败页能修改 Docker/NAS 地址；一旦进入离线 App，设置页没有明确入口重新打开服务器地址设置，对家庭用户不够可靠。

已完成：

- Android `HomeNoteAndroid` bridge 新增 `openServerSettings()`，可从 WebView 页面重新打开原生服务器地址设置页。
- 前端设置页在 APK 环境下显示“修改手机端服务器地址”，用于离线使用后重新填写 Docker/NAS 局域网地址。
- 普通浏览器不显示该入口，不影响 Web 端使用。
- 新增/更新 Android wrapper 和前端静态测试，防止服务器地址入口以后被误删。
...
- `npm.cmd run android:verify`：通过，APK 内置最新 `assets/www/index.html` 和前端 JS/CSS。
- `npm.cmd run android:delivery-check`：通过，包含临时 HTTP smoke。
- `npm.cmd run android:device-smoke`：当前电脑未检测到 USB 手机，因此真机烟测未完成；不应声称 vivo / Huawei 已验证。

下一步：

- 连接真实手机后跑 `npm.cmd run android:device-smoke`。
- 仍需用户在 vivo X300 Pro 和 Huawei P30 Pro / HarmonyOS 上做真机验收：离线新建 / 编辑 / 重启持久化 / 修改服务器地址 / 恢复联网同步。

## 2026-07-06 Android 路线纠错：停止 WebView 壳，改为原生离线核心

用户明确指出：需要的是能长期离线使用的 Android App，不是几百 KB 的 WebView / file:// 壳 APK。此前围绕 WebView、IndexedDB、服务器地址桥、离线壳的 Gate 1-14 不能作为最终 Android 交付结论，后续不得再沿这条路线继续修补。

本轮已把 Android debug APK 改成原生 Android 页面 + 手机本地 SQLite：

- `MainActivity.java` 不再使用 `WebView`，不再加载 `file:///android_asset/www/index.html`。
- APK 不再打包 `assets/www` 前端资源。
- 手机本地数据库为 `home_note_native.db`。
- 当前原生离线核心支持：首页列表、新建记录、编辑记录、详情查看、保存到手机本地。
- 保存不依赖 Docker/NAS；没有服务器也可以写入手机本地 SQLite。

当前不能夸大：

- 这不是完整最终 Android App。
- 原生端还没有迁入 Web 端的富文本完整能力、附件、Note Station `.nsx` 导入、搜索筛选、分类管理、成员、同步到 Docker/NAS 等功能。
- 当前电脑没有连接真实手机，因此不能声明 vivo X300 Pro 或 Huawei P30 Pro / HarmonyOS 真机已通过。

验证结果：

- `npm.cmd run android:delivery-check`：通过。
- 其中包含 `npm.cmd run check`、`npm.cmd run test`、`npm.cmd run build`、`npm.cmd run android:build`、`npm.cmd run android:verify`、临时 `http://127.0.0.1:3400` HTTP smoke。
- `npm.cmd run android:verify` 输出 `nativeOffline=true`，`webAssetCount=0`，确认当前 APK 不含 WebView 离线网页壳资源。
- `npm.cmd run android:device-smoke`：未通过，原因是当前电脑没有检测到可用 USB 手机；不能声称 vivo / Huawei 真机已通过。

后续 Android 开发顺序：

1. 先用真实手机安装当前原生离线核心 APK，确认不连接 Docker/NAS 时能新建、编辑、退出重开后仍在。
2. 再逐步把搜索、分类、富文本、附件、Note Station 导入、Docker/NAS 同步迁进原生端。
3. 每一步都必须真机构建验证；没有真机验证时不能交付给用户说“可用”。

## 2026-07-06 原生离线 Android：本地搜索和分类筛选

继续沿原生 Android + 手机本地 SQLite 路线推进，不回到 WebView / file:// 壳方案。

本轮完成：

- 原生首页新增搜索框：可按标题、正文、标签搜索本机 SQLite 记录。
- 原生首页新增分类筛选：包含“全部分类”和本地已有分类。
- 新增“清除筛选”入口，方便回到全部记录。
- SQLite 查询新增 `listNotes(String searchQuery, String categoryFilter)`。
- SQLite 新增 `listCategories()`，按本地记录里的分类生成筛选项。
- Android 静态测试新增原生搜索 / 分类筛选覆盖，防止回退成只有列表。

验证：

- `node --test tests/android-wrapper.test.js`：通过，6 tests。
- `npm.cmd run check`：通过，SQLite `integrityCheck=ok`。
- `npm.cmd run test`：通过，16 suites / 86 tests / 86 pass。
- `npm.cmd run build`：通过。
- `npm.cmd run android:build`：通过，重新生成原生 APK。
- `npm.cmd run android:verify`：通过，`nativeOffline=true`，`webAssetCount=0`。
- `npm.cmd run android:delivery-check`：通过，覆盖 check/test/build/android build/android verify/HTTP smoke。
- `npm.cmd run android:device-smoke`：未通过，原因是当前电脑没有检测到 USB 手机；不能声明 vivo / Huawei 真机已通过。

仍未完成：

- 原生端还没有 Docker/NAS 同步。
- 原生端还没有富文本、附件、Note Station `.nsx` 导入、成员体系完整迁移。
- 原生 UI 仍是核心可用版，不是最终视觉还原版。

## 2026-07-06 原生离线 Android：本机自定义分类管理

继续沿原生 Android + 手机本地 SQLite 路线推进，不回到 WebView / file:// 壳方案。

本轮完成：

- Android 原生本地数据库版本升级到 2。
- 新增手机本地 `categories` 表，用于保存自定义分类。
- 初始化 11 个默认分类：家庭事务、房屋 / 设备、维修 / 售后、购物 / 消费、证件 / 账号、孩子 / 教育、老人 / 健康、宠物、工作 / 杂事、临时记录、未分类 / 待整理。
- 首页新增“分类”入口，进入原生分类管理页。
- 分类管理页支持新增分类，并可一键按分类筛选回首页。
- 新建 / 编辑记录页的分类输入旁增加本地分类快捷选择。
- 新建或编辑记录时会自动确保分类写入本机 `categories` 表。
- Android 静态测试新增本机自定义分类覆盖，防止回退成只能按已有记录推导分类。

验证：

- `node --test tests/android-wrapper.test.js`：通过，7 tests。
- `npm.cmd run android:delivery-check`：通过，覆盖 `check`、`test`、`build`、`android:build`、`android:verify` 和临时 HTTP smoke。
- `npm.cmd run check`：通过，SQLite `integrityCheck=ok`。
- `npm.cmd run test`：通过，16 suites / 87 tests / 87 pass。
- `npm.cmd run build`：通过，仍有已知 Vite chunk size warning。
- `npm.cmd run android:verify`：通过，`nativeOffline=true`，`hasLauncherIcon=true`，`webAssetCount=0`。
- `npm.cmd run android:device-smoke`：未通过，原因是当前电脑没有检测到可用 USB 手机；不能声称 vivo / Huawei 真机已通过。

仍未完成：

- 原生端还没有 Docker/NAS 同步。
- 原生端还没有富文本、图片、附件、Note Station `.nsx` 导入。
- 原生端还没有成员体系完整迁移。
- 原生 UI 仍是核心可用版，不是最终视觉还原版。

下一步建议：

1. 继续补原生端同步队列表和服务器地址配置。
2. 再实现恢复联网后手动同步到 Docker/NAS。
3. 真机连接后必须跑 `npm.cmd run android:device-smoke`，并在 vivo X300 Pro 与 Huawei P30 Pro / HarmonyOS 上人工验收。

## 2026-07-06 原生离线 Android：同步前置结构

继续沿原生 Android + 手机本地 SQLite 路线推进，不回到 WebView / file:// 壳方案。

本轮完成的是“恢复联网同步”的前置能力，不是完整联网同步：

- Android 原生本地数据库版本升级到 3。
- 新增 `sync_queue` 表，用来记录待同步的新建 / 编辑记录。
- 新建记录时写入 `sync_queue` 的 `create` mutation。
- 编辑记录时写入 `sync_queue` 的 `update` mutation。
- 首页离线提示增加“待同步：N 条”。
- 首页增加“同步”入口。
- 新增原生“离线同步”页面：
  - 可填写并保存 Docker/NAS 服务器地址到 `SharedPreferences`。
  - 显示当前待同步数量。
  - 提供“手动同步”按钮，但当前只明确提示“同步功能下一阶段接入 Docker/NAS”。

重要边界：

- 当前还没有真正向 Docker/NAS 发起网络同步请求。
- 当前不会把手机本地记录上传到服务器。
- 当前只是让离线记录具备后续同步所需的本地队列和服务器地址配置基础。

验证：

- 先新增失败测试，确认同步前置能力缺失时 `tests/android-wrapper.test.js` 会失败。
- 实现后 `node --test tests/android-wrapper.test.js`：通过，8 tests。
- `npm.cmd run android:delivery-check`：通过，覆盖 `check`、`test`、`build`、`android:build`、`android:verify` 和临时 HTTP smoke。
...
1. 实现原生端对 Docker/NAS 的最小同步请求：先只支持上传本机新建记录。
2. 同步成功后把对应 `sync_queue` 项标记为 done。
3. 同步失败时保留 pending 状态，并在同步页提示失败。
4. 再补编辑记录同步和冲突提示。

## 2026-07-06 原生离线 Android：最小新建记录同步

继续沿原生 Android + 手机本地 SQLite 路线推进，不回到 WebView / file:// 壳方案。

本轮完成的是最小联网同步闭环的第一步：只同步手机本地新建记录到 Docker/NAS 服务端。

完成内容：

- 原生 Android 使用 `HttpURLConnection` 在后台线程执行同步，避免主线程联网导致老安卓 / HarmonyOS 崩溃。
- 原生同步页的“手动同步”不再只是占位提示，会读取已保存的服务器地址。
- 对 `sync_queue` 中 `create` 类型的 pending / failed 记录逐条 POST 到真实服务端接口 `/api/notes`。
- 同步请求使用 JSON：title、content、contentText、categoryId、memberId=self、noteType=normal、sourceType=manual、tags。
- 同步成功后调用 `markSyncDone`，把队列项标记为 done。
- 同步失败后调用 `markSyncFailed`，状态为 failed；`pendingSyncCount()` 仍统计 pending 和 failed，避免失败项从用户视野消失。
- 修复上轮遗漏：全新安装数据库时 `onCreate()` 现在会创建 `sync_queue` 表，升级路径 `oldVersion < 3` 也会创建。
...
- 原生端编辑记录同步还没有做。
- 同步冲突处理还没有做。
- 富文本、图片、附件、Note Station `.nsx` 导入仍未迁入原生端。
- 没有 USB 真机连接，因此还不能声称 vivo / Huawei 真机已通过。

验证：

- 先新增失败测试，确认缺少 `HttpURLConnection` 时 Android 同步测试失败。
- 实现后 `node --test tests/android-wrapper.test.js`：通过，9 tests。
- `npm.cmd run android:delivery-check`：通过，覆盖 `check`、`test`、`build`、`android:build`、`android:verify` 和临时 HTTP smoke。
- `npm.cmd run test`：通过，16 suites / 89 tests / 89 pass。
- `npm.cmd run android:verify`：通过，`nativeOffline=true`，`hasLauncherIcon=true`，`webAssetCount=0`。
- `npm.cmd run android:device-smoke`：未通过，原因是当前电脑没有检测到可用 USB 手机。

下一步：

1. 接入真实手机后验证：离线新建 -> 填服务器地址 -> 手动同步 -> Docker/NAS 浏览器端看到记录。
2. 实现原生端编辑记录同步。
3. 增加同步失败详情和重试提示。
4. 再考虑冲突处理。

## 2026-07-07 原生离线 Android：远端 ID 与编辑同步

继续沿原生 Android + 手机本地 SQLite 路线推进，不回到 WebView / file:// 壳方案。

本轮补齐的是原生端恢复联网同步的第二步：同步成功后保存服务端记录 ID，并允许后续编辑同步回 Docker/NAS。

完成内容：

- Android 原生本地数据库版本升级到 4。
- `notes` 表新增 `remote_id` 字段，用来保存 Docker/NAS 服务端返回的 `note.id`。
- 新建记录保存到手机本地时会写入 `sync_queue` 的 `create` mutation。
- 编辑记录保存到手机本地时会写入 `sync_queue` 的 `update` mutation。
- 如果一条本地记录还存在待同步 create，继续编辑时不额外创建 update；create 同步时会读取最新本地内容，避免重复同步或 PATCH 不存在的远端记录。
- `POST /api/notes` 成功后解析响应里的 `note.id`，保存到本地 `remote_id`。
- 已有 `remote_id` 的本地编辑记录会通过 `PATCH /api/notes/:remoteId` 同步到 Docker/NAS。
- 如果 update mutation 没有 `remote_id`，会保留失败状态并提示“没有远端记录 ID，先同步新建记录”，避免静默丢数据。

验证：

- 先新增失败测试 `stores remote note ids and syncs native offline edits back to Docker NAS`，确认缺少 v4/remote_id/PATCH 时失败。
...
1. 真实手机验证：离线新建 -> 手动同步 -> Web 端看到记录 -> 手机端编辑 -> 再同步 -> Web 端看到更新后的最终内容。
2. 增加原生同步失败详情：失败原因、最近失败时间、重试入口更清晰。
3. 增加基础冲突提示：服务端记录已变化时，不静默覆盖。
4. 再迁入原生端富文本、图片、附件和 Note Station `.nsx` 导入。

## 2026-07-07 原生离线 Android：同步失败详情

继续沿原生 Android + 手机本地 SQLite 路线推进。

本轮补齐的是同步失败可见性：用户不应该只看到“待同步 N 条”，还要能知道失败原因，便于判断是服务器地址错、网络断开、服务端返回错误，还是记录缺少远端 ID。

完成内容：

- Android 原生本地数据库版本升级到 5。
- `sync_queue` 表新增 `error_message` 和 `last_attempt_at` 字段。
- 旧数据库升级时会自动补齐这两个字段。
- 同步成功时清空失败原因并记录最后尝试时间。
- 同步失败时保存失败原因和最后尝试时间。
- 原生“离线同步”页会展示最近同步失败项，包括记录标题、同步类型、失败原因和最后尝试时间。

验证：

- 先新增失败测试 `shows native sync failure details for retry decisions`，确认缺少 v5/错误字段/失败展示时失败。
...
1. 真实手机验证失败详情：配置错误服务器地址 -> 手动同步 -> 同步页显示失败原因；改回正确地址后重试。
2. 补基础冲突提示：服务端记录已变化时，不静默覆盖。
3. 再迁入原生端富文本、图片、附件和 Note Station `.nsx` 导入。

## 2026-07-07 原生离线 Android：编辑同步冲突保护

继续沿原生 Android + 手机本地 SQLite 路线推进，不回到 WebView / file:// 壳方案。

本轮补齐的是原生端编辑同步的底线保护：如果手机离线编辑期间，Docker/NAS 上同一条记录已经被别的设备更新，手机端同步时不能静默覆盖服务端内容。

完成内容：

- Android 原生本地数据库版本升级到 6。
- `notes` 表新增 `remote_updated_at` 字段，用来保存最近一次从服务端同步成功时的更新时间。
- create 同步成功后会同时保存服务端 `note.id` 和 `note.updatedAt`。
- update 同步时会把 `baseUpdatedAt` 一起提交给服务端。
- 服务端已有 `baseUpdatedAt` 冲突校验：当服务端记录更新时间不等于手机端基准时间时，返回 `409 note_conflict`。
- Android 原生端遇到 `409` 或 `note_conflict` 时，会把该队列项保留为 failed，并显示“记录已经在其他设备更新，请先确认后再同步”，避免假装同步成功。
- 旧的本地数据库升级时会自动补齐 `remote_updated_at` 字段。

验证：

- 先新增失败测试 `sends baseUpdatedAt to avoid silently overwriting server edits`，确认缺少 v6、`remote_updated_at`、`baseUpdatedAt` 和冲突提示时失败。
...
- `npm.cmd run android:build`：通过，重新生成 `android/app/build/outputs/apk/debug/app-debug.apk`。
- `npm.cmd run android:verify`：通过，`nativeOffline=true`、`hasLauncherIcon=true`、`webAssetCount=0`。
- `npm.cmd run android:delivery-check`：通过，包含 check、test、build、Android build/verify 和临时 HTTP smoke。
- `npm.cmd run android:device-smoke`：未通过，原因是当前电脑没有检测到可用 USB 手机；不能声称 vivo / Huawei 真机已通过。

下一步：

1. 真实手机验证：离线新建 -> 同步到 Docker/NAS -> Web 端修改同一条记录 -> 手机端离线编辑后再同步 -> APK 同步页应显示冲突失败，而不是覆盖服务端内容。
2. 原生端日常编辑能力继续补富文本、图片、附件。
3. 最后迁入原生端 Note Station `.nsx` 导入。

## 2026-07-07 原生离线 Android：记录归档 / 删除生命周期

用户明确指出当前原生 APK 功能量太少，不能把字段级小修包装成完整交付。后续开发按“可用闭环”推进。

本轮补齐的是原生端记录生命周期：离线状态下不只可以新建和编辑，也可以从日常列表中归档或删除记录，并在恢复联网后同步到 Docker/NAS。

完成内容：

- Android 原生本地数据库版本升级到 7。
- `notes` 表新增 `is_archived` 和 `is_deleted` 字段。
- 首页记录列表默认过滤已归档和已删除记录。
- 详情页新增“记录操作”，包含“归档记录”和“删除记录”。
- 归档记录会本地标记 `is_archived=1`，并写入 `sync_queue` 的 `archive` mutation。
- 删除记录会本地标记 `is_deleted=1`，并写入 `sync_queue` 的 `delete` mutation。
- 如果本地新建记录尚未同步就被删除，会直接清理本机记录和待同步 create 队列，避免产生必然失败的远端 delete。
- 如果本地新建记录尚未同步就被归档，只从本机列表隐藏，不生成无远端 ID 的 archive 同步失败项。
- 恢复联网后：
  - `archive` mutation 调用 `POST /api/notes/:id/archive`；
  - `delete` mutation 调用 `DELETE /api/notes/:id`。

验证：

- 先新增失败测试 `supports native offline archive and delete lifecycle with sync`，确认缺少 v7、生命周期字段、归档/删除 UI 和同步请求时失败。
...
1. 用户真机验证：离线新建、编辑、归档、删除、搜索、分类筛选。
2. 验证已同步记录归档/删除后，Docker/NAS Web 端是否同步隐藏。
3. 继续补原生端日常可用能力：更好的标签筛选、富文本、图片/附件。
4. 最后迁入原生端 Note Station `.nsx` 导入。

## 2026-07-07 原生离线 Android：标签筛选与快速标签

继续按“可用闭环”推进原生离线 APK，不回到 WebView / file:// 壳方案。

本轮补齐的是离线记录的标签组织能力：记录不只是能写，还能通过家庭常用标签快速标记和找回。

完成内容：

- 原生首页新增标签筛选状态 `currentTagFilter`。
- 首页筛选区新增“全部标签”和已有标签 chip。
- 点击标签 chip 后，列表只显示包含该标签的本机记录。
- “清除筛选”会同时清空关键词、分类和标签筛选。
- 编辑页在标签输入框下新增常用标签按钮：`待办`、`重要`、`维修`、`账单`。
- 编辑页新增“清空标签”按钮。
- 保存记录时会通过 `normalizeTags` 去重并规范化标签分隔。
- 本机数据库新增 `listTags()`，从未删除、未归档记录中提取标签。
- 同步到 Docker/NAS 时继续使用已有 `tags` JSON 数组，不改变服务端接口。

验证：

- 先新增失败测试 `supports native offline tag chips and quick tag editing`，确认缺少标签状态、标签 chip、快速标签、清空标签和标签筛选 SQL 时失败。
...
1. 用户真机验证：新建记录时点常用标签 -> 保存 -> 首页出现标签 chip -> 点标签筛选能找回记录。
2. 继续补原生端富文本基础能力。
3. 再补图片 / 附件本地保存和同步。
4. 最后迁入原生端 Note Station `.nsx` 导入。

## 2026-07-07 原生离线 Android：成员归属与筛选

继续按“家庭自用、离线可用”的方向推进原生 APK，不再回到 file:// WebView 壳方案。

本轮补齐的是两人家庭使用的基础归属能力：离线记录不再永远硬编码成“我”，而是支持默认成员“我 / 爱人”的选择、展示、筛选和同步。

完成内容：

- Android 原生本地数据库版本升级到 8。
- `notes` 表新增 `member_id TEXT NOT NULL DEFAULT 'self'`。
- 老本机库升级时会自动补 `member_id`，默认归到“我”。
- 首页筛选区新增“全部成员 / 我 / 爱人”。
- 首页记录卡片显示成员归属。
- 新建 / 编辑记录页新增“当前成员”，可在“我 / 爱人”之间切换。
- 详情页显示“成员：我 / 爱人”。
- 同步到 Docker/NAS 时，`memberId` 从本地记录读取，不再硬编码为 `self`。
- 默认成员仍只保留“我 / 爱人”，没有新增真实成员管理功能，也没有写回旧称呼。

验证：

- 先新增失败测试 `supports native offline member ownership and filtering for family use`，确认缺少成员字段、成员筛选、编辑页成员选择和同步成员 payload 时失败。
...
1. 用户真机验证：新建记录选择“爱人” -> 保存 -> 首页按“爱人”筛选 -> 同步到 Docker/NAS。
2. 继续补原生端富文本基础能力、图片 / 附件本地保存和同步。
3. 最后迁入原生端 Note Station `.nsx` 导入。

## 2026-07-07 原生离线 Android：基础富文本格式

继续把原生 APK 从“能写纯文本”推进到“日常记录够用”。本轮没有引入大型编辑器，也没有回到 WebView 壳；采用轻量 Markdown 风格标记保存，详情页在本机安全渲染。
...
- 支持按钮：
  ...
  - `列表`：插入 `- ` 行前缀。
  - `待办`：插入 `- [ ] ` 行前缀。
- 详情页改用原生 `SpannableStringBuilder` 渲染基础格式：
  - `**粗体**` -> 粗体；
  - `*斜体*` -> 斜体；
  - `__下划线__` -> 下划线；
  - `~~删除线~~` -> 删除线；
  ...
  - `- [ ] 待办` -> `☐`；
  - `- [x] 已完成` -> `☑`。
- 不改服务端接口，不改同步协议，不改当前数据库结构；格式内容仍保存在 `notes.content`，搜索和同步继续按文本处理。

验证：

- 新增失败测试 `supports native offline basic rich text formatting for daily notes`。
...
1. 用户真机验证：断网新建记录，使用加粗 / 标题 / 待办按钮，保存后详情页确认格式可见。
2. 继续补原生端图片 / 附件本地保存和同步。
3. 最后迁入原生端 Note Station `.nsx` 导入。

## 2026-07-07 原生离线 Android：本机附件 / 图片保存

- 原生 Android APK 已新增本机附件保存能力，不依赖 Docker/NAS 即可在记录详情页点击“添加附件 / 图片”，通过系统文件选择器选择图片或文件。
- 附件文件复制到 Android App 私有目录 `getFilesDir()/attachments`，SQLite 只保存 `note_attachments` 元数据：记录 ID、文件名、本机路径、MIME、大小、创建时间。
- 数据库版本从 8 升到 9，新增 `note_attachments` 表，并保留现有原生离线记录、分类、标签、成员、同步队列结构。
- UI 沿用当前原生 Android 卡片和绿色主色，不做视觉重做；详情页新增“本机附件”卡片。
- 已验证：`npm.cmd run android:delivery-check` 通过，包含 check、test、build、android:build、android:verify 和本机 HTTP smoke。
- `npm.cmd run android:device-smoke` 未通过，原因是当前没有检测到 USB 调试手机；需要用户连接真机后再跑。
- 当前未完成：附件/图片同步到 Docker/NAS、附件在不同手机间同步、原生端 Note Station `.nsx` 导入。

## 2026-07-07 原生离线 Android：本机附件同步到 Docker/NAS

- 原生 Android APK 继续沿用当前家事记原生界面风格，没有重做 UI、没有改卡片/导航/配色方向。
- 本机附件表升级到 v10：`note_attachments` 新增 `remote_id`、`sync_status`，旧本机库升级时自动补字段。
- 离线添加附件 / 图片后，附件文件仍先复制到手机 App 私有目录；恢复联网并手动同步时，会把未同步附件编码到记录创建 / 更新 payload，通过现有 Docker/NAS notes API 写入服务端附件目录。
- 同步成功后会把该记录本机附件标记为 `synced`，避免下一次同步重复上传。
- 新增测试锁定：本机附件同步 payload、Base64 文件读取、服务端附件字段、成功后标记已同步。

验证：

- `node --test tests/android-wrapper.test.js`：通过，18 tests。
- `npm.cmd run android:build`：通过，重新生成 `android/app/build/outputs/apk/debug/app-debug.apk`。
- `npm.cmd run android:delivery-check`：通过，包含 `check`、`test`、`build`、`android:build`、`android:verify`、临时 HTTP smoke。
- `npm.cmd run android:device-smoke`：未通过，原因是当前电脑没有检测到 USB 调试手机；需要用户真机安装验证。

下一步：

1. 真机验证：断网新建记录 -> 添加图片/文件 -> 关闭重开仍存在 -> 填 Docker/NAS 地址 -> 手动同步 -> 浏览器端确认附件进入服务端。
2. 继续做原生 APK 的服务端附件下载 / 打开体验，或者迁入原生端 `.nsx` 导入，按用户优先级决定。

---

## 2026-07-07 离线 Android App 第一阶段基础完成

- 已停止 49KB WebView 壳路线，本轮改为 Capacitor 打包本地 React + Vite 资源。
- `capacitor.config.json` 使用 `webDir=dist`，APK 内包含 `assets/public/index.html`、打包后的 JS/CSS 和 `assets/capacitor.config.json`，不再以启动远程 NAS URL 作为 App 形态。
- Android 入口已切换为 Capacitor `BridgeActivity`，并接入 `@capacitor-community/sqlite`、`@capacitor/filesystem`、`@capacitor/preferences`。
- 新增 `src/data/local/` 本地数据层：SQLite schema、notes repository、attachments repository、sync_queue repository。
- 前端离线存储层已开始接入 Android SQLite：本地 notes 读取、保存、归档、删除和待同步队列会优先写入本地层；浏览器环境继续保留 IndexedDB 适配。
- Android APK 构建环境已修正：启用 AndroidX / Jetifier，minSdk 调整为 24，app 模块显式依赖 `capacitor-android` 和 AppCompat。
- 当前 APK：`android/app/build/outputs/apk/debug/app-debug.apk`，大小 `25,393,803 bytes`，`npm.cmd run android:verify` 已确认不是 49KB 壳。
- 本轮没有提交 `data/`、数据库、附件、备份、导出、`.nsx` 或日志。
- 尚未完成：真机飞行模式实测、SQLite 真机运行日志验证、NAS 手动同步 push/pull、附件本地文件系统完整同步。下一步应先做真机安装和离线新建/编辑/删除/归档实测，再进入同步服务。

---

## 2026-07-07 Android APK 交付检查补充

- 已运行 `npm.cmd run android:delivery-check`，完整通过：check、test、build、android:build、android:verify、临时 3400 服务 HTTP smoke。
- `android:delivery-check` 生成并验证的 APK 仍为 `25,393,803 bytes`，`nativeShellOnly=false`。
- 已尝试 `npm.cmd run android:device-smoke`，但当前电脑没有检测到 USB 连接且授权的手机，因此真机飞行模式测试仍未完成。
- 当前下一步不是继续做壳，而是安装当前 APK 在真实手机上测：飞行模式打开、本地新建、本地编辑、本地删除 / 归档、重启后保留。通过后再推进 NAS 手动同步。

---

## 2026-07-07 Android 离线本地数据层补强

- 本轮继续沿着“真正离线可用 Android App”目标推进，不回到 49KB WebView 壳路线。
- Android 本地 SQLite schema 已补齐 categories、members、tags 表；此前原生 SQLite 主要覆盖 notes、attachments 和 sync_queue。
- 新增本地仓储：localCategoriesRepository.js、localMembersRepository.js、localTagsRepository.js。
- localAttachmentsRepository.js 新增批量写入和读取附件元数据能力。
- offlineStore 保存服务端 / 本地快照时，会把分类、成员、标签、附件元数据同步写入 Android SQLite；纯离线读取时也优先从 Android SQLite 恢复这些基础资料，并把附件挂回对应 note。
- 浏览器 IndexedDB 路径保持可用，并补齐 attachments 快照写入 / 返回，避免浏览器和 Android 路径行为分叉。
- 本轮仍未宣称真机飞行模式最终通过：当前证据来自静态/单元测试、前端构建和本地 APK 验证；真机需要用户安装后继续测。

---

## 2026-07-07 Android 本地数据诊断入口

- 设置页新增“本地数据诊断”卡片，用于真机测试时查看手机本地数据状态，不需要连接 Docker/NAS。
- 诊断信息来自 getLocalStoreDiagnostics()，包含：本地库状态、记录数、附件数、分类数、标签数、待同步队列数量和最近本地库错误。
- 这是为了减少用户真机测试时只能靠肉眼猜测的问题；如果华为 / vivo 上出现离线异常，可以先看设置页诊断卡，再决定是否需要 ADB 日志。
- 本轮没有改变业务数据，不提交 APK、数据库、附件或运行数据。

---

## 2026-07-08 09:02 Android 服务器地址设置修复

- 用户真机测试发现：离线 APK 设置页找不到连接 NAS/Docker 的服务器地址入口。
- 根因：前端只在 window.HomeNoteAndroid.openServerSettings 存在时显示“修改手机端服务器地址”，但当前 Capacitor MainActivity 没有注入 HomeNoteAndroid bridge，导致入口被隐藏。
- 修复方向：不要依赖 native bridge 才能配置服务器。前端新增 ANDROID_SERVER_URL_STORAGE_KEY，使用 localStorage 保存服务器地址；没有 native bridge 时通过 App 内 prompt 输入地址。
- 用户输入 192.168.x.x:3300 会自动补成 http://192.168.x.x:3300。
- apiUrl() 已改为 Android native 或 file: 离线模式都优先使用保存的 NAS/Docker 地址，避免保存了地址但请求仍打向本地壳。
- 经验：以后 Android 关键配置必须提供前端 fallback，不能只写文档声称有 native 设置入口。
- 验证：npm.cmd run android:delivery-check 通过，覆盖 check/test/build/android:build/android:verify/HTTP smoke。
- 新 APK 路径：android/app/build/outputs/apk/debug/app-debug.apk，大小 25,707,473 bytes；APK 文件仍不提交 Git。

---

## 2026-07-08 10:30 Android APK 连接 NAS Docker CORS 修复

- 用户反馈：离线 APK 设置家庭 NAS / Docker 服务地址后仍无法连接。
- 根因验证：对用户 NAS Docker 的 /api/health 发送 Origin: capacitor://localhost，服务端返回 200，但 Access-Control-Allow-Origin 和 Access-Control-Allow-Credentials 为空。
- 结论：Docker 页面同源访问可以正常用，但 APK 内置页面请求远程 NAS Docker 是跨域请求，旧服务端未允许 capacitor://localhost / Android 本地 Origin，导致 WebView 拦截。
- 修复：Express 入口增加 CORS 中间件，默认允许 capacitor://localhost、ionic://localhost、localhost / 127.0.0.1 Origin，并支持 NOTE_CORS_ORIGINS / NOTE_CORS_ORIGIN 扩展；OPTIONS 预检返回 204；允许 Content-Type、X-File-Name、X-Member-Id、Authorization。
- 前端 fetchApi 统一使用 credentials: include，为后续访问口令 / cookie 场景保留能力。
- 经验：以后 APK 连接远程 Docker 的验收必须模拟 Origin: capacitor://localhost，不只测浏览器同源页面。
...
## 2026-07-08 Note Station 大文件 .nsx 导入修复记忆

- 用户确认 NAS Docker 运行的是此前提供的 GHCR 镜像：ghcr.io/ddrcpddr/note:100375f4a8322655fab61c1d0e87af5b47cf3344。
- 健康接口确认该镜像 build.commit 为 100375f4a8322655fab61c1d0e87af5b47cf3344；问题不是用户拉错版本。
- 远程 HTTP smoke 的小型合成 Note Station 导入通过，但真实 22MB 级 .nsx 上传 dry-run 在长请求中 ECONNRESET。
- 本地解析同一个真实 .nsx 成功：93 条记录，0 失败，20 个附件；说明 parser 本身可识别该文件。
- 根因：旧 Web 导入把上传和解析放在一个同步 HTTP 请求里，真实大文件在 NAS / 域名链路上容易超时或被重置。
- 修复方向：.nsx Web 上传进入异步 preview。服务端先保存文件并返回 processing，后台 dry-run 解析并缓存预览；前端轮询 importId，解析完成后才允许确认导入。
- 本次修复必须等待新 GHCR 镜像构建完成后，用户 NAS 重新拉取新镜像再验证真实 .nsx。
- 记住：以后验证 Docker/GHCR 问题时，必须用用户实际拉取的镜像 commit 和真实大文件路径做区分，不能只用小型合成 smoke 就宣称真实导入可用。

## 2026-07-08 Note Station NSX 富文本详情回归修复

- 用户在服务端 Docker 中重新导入真实 .nsx 后反馈：详情页仍显示纯文本，图片 / 附件没有进入富文本正文。
- 复查真实 .nsx：93 条记录均带 HTML；“用户截图中的某条导入记录”含表格和粗体；带图片的记录也有可内联附件。问题不是 .nsx 没格式。
- 本地临时库完整导入验证：确认导入后 API 能生成 content_html / source_html / richContent，图片记录能生成 /api/attachments/.../file 内联引用。
- 本轮加固：异步 NSX 后台预览缓存也保存 includeContent + includeRawContent；前端 normalizeNote 在旧缓存只有 contentHtml、没有 richContent 时用 contentHtml 兜底展示富文本。
- 回归测试已补：异步 .nsx 上传 -> 轮询 previewed -> commit -> /api/notes?id=... 必须返回 richContent、inline 图片引用和 inline 附件标记。
- 以后改 Note Station 导入时，不能只测 dry-run / preview / 小型 smoke；必须测确认导入后的详情 API 和前端详情页富文本、内联附件。

## 2026-07-08 Android APK 富文本附件图片 URL 修复

- 用户真机反馈：APK 已能和 Docker 同步，但 Note Station 导入记录正文里的图片显示为裂图，只剩附件文件名；网页端同源页面更容易正常显示。
- 根因定位：服务端富文本 HTML 中的内联图片地址是 `/api/attachments/<id>/file` 根相对路径。浏览器访问 Docker 页面时会解析到当前 Docker 域名；Android / Capacitor APK 内置页面的 origin 是本地 App，因此相对地址不会自动指向用户配置的 Docker/NAS 地址。
- 修复：详情页渲染富文本前，将正文内 `/api/attachments/.../file` 的 `img.src` 和附件下载 `href` 通过 `apiUrl(...)` 改写到当前配置的服务端地址；没有配置服务端时保持原 HTML，不写回数据库。
- 回归测试：`tests/frontend-ui.test.js` 增加富文本附件图片 URL 改写断言，避免之后又把 APK 下图片预览修坏。
- 经验：以后排查“网页正常、APK 图片/附件打不开”时，优先检查富文本 HTML 和附件链接是否仍是 `/api/...` 相对路径，而不是只查 NSX parser。

## 2026-07-09

### Note Station Docker 导入卡住和富文本图片坏图修复

- 用户反馈 GHCR / NAS Docker 中 `.nsx` 导入会卡在“正在解析”，且导入后的 Note Station 图片仍以 `ns_attach_image_*.png/.jpg` 形式显示坏图。
- 根因一：异步 NSX preview 只在上传请求时启动一次，容器重启或后台 worker 丢失后，数据库中的 `imports.status=processing` 不会自动恢复。
- 根因二：真实 Note Station 图片标签可能通过 `src`、`alt`、`title` 或文件名引用附件，旧逻辑只按 `ref` 匹配，不能覆盖所有导出形态。
- 已修复：`getImportPreview()` 遇到 `processing` NSX 批次会重新排队后台解析；失败标记增加兜底；web 导入和正式导入脚本统一按多候选引用匹配图片附件。
- 已补测试：NSX web import fixture 覆盖 `ns_attach_unmatched_2.jpg` 文件名引用；新增持久化 processing 批次恢复测试，避免再出现“正在解析”永久卡住。
- 已验证：`npm.cmd run check`、`npm.cmd run test`、`npm.cmd run build` 通过；本机构建 `note:nsx-import-fix-test` Docker 镜像并启动临时容器，HTTP smoke 通过，容器内导入记录详情确认富文本图片映射到 `/api/attachments/<id>/file`，没有残留 `ns_attach_*` 坏图地址。
- 交付原则更新：涉及 Docker/GHCR 或 APK 的用户反馈，不能只跑源码测试；至少要做对应产物级验证，无法验证时必须明确说明，不能把“代码通过”等同于“交付通过”。

## 2026-07-09 Note Station 旧附件 ID 与 NAS 404 回归修复

- 用户通过浏览器网络面板定位：本地临时 Docker 的 `/api/attachments/.../file` 返回 200，但 NAS 域名同类附件请求返回 404；这说明不能只看页面是否有 `<img>`，必须验证附件 URL 实际状态码。
- 只读诊断 NAS `/api/app-data` 后确认：当前服务端返回的 Note Station 记录附件表和富文本引用可以一致；用户页面出现旧 `attachment_mrbw...` 404，更像旧 `content_html` / 浏览器缓存 / 旧导入状态残留被前端或服务端继续信任。
- 根因加固：`buildRichContentFromNote()` 不再无条件信任 `content_html`。如果 `content_html` 中的 `/api/attachments/{id}/file` 或 `data-attachment-id` 不属于当前笔记附件表，则回退到 `source_html + 当前 attachments` 重新生成富文本图片引用。
- 运行时 Note Station 图片匹配规则已和导入器对齐：按 `ref`、`src`、`alt`、`title`、原始文件名、source path 多候选匹配，避免旧库运行时修复只按 `ref` 漏图。
- 前端 `fetchApi()` 默认增加 `cache: 'no-store'`，避免浏览器 / 代理 / PWA 继续拿旧 app-data 或旧详情响应。
- 新增回归测试：旧 `content_html` 指向不存在附件 ID 时必须用当前附件重新生成；当前附件 ID 有效时仍保留用户编辑后的 `content_html`，不能误覆盖。
- Docker 产物级验证：用当前源码构建 `note:stale-attachment-fix-test`，空数据容器上传真实 `.nsx`，预览 93 条 / 成功 93 条，确认导入 93 条；4 条带图片记录共 20 个内联图片引用，全量 GET `/api/attachments/{id}/file` 为 200，`brokenImageRefs=0`。
- 验证坑记录：PowerShell `Invoke-WebRequest -OutFile` 可能不返回响应对象，不能用 `$resp.StatusCode` 判断是否 broken；附件状态检查应直接读取响应对象或单独检查 HTTP 状态，避免误报所有图片 broken。

### GHCR 发布镜像复测补充

- GitHub Actions 已完成 `f05896f9de526a0d391918176209225133a340b8` 的 Docker 镜像构建，状态 success。
- 已拉取并运行 `ghcr.io/ddrcpddr/note:f05896f9de526a0d391918176209225133a340b8`，不是本地源码镜像。
- 在 GHCR 精确 tag 容器中上传真实 `.nsx`：预览 93 条 / 成功 93 条 / 导入 93 条；4 条记录含图片，20 个内联图片引用；全量 GET `/api/attachments/{id}/file` 后 `brokenImageRefs=0`。
- 给用户 NAS / 群晖 / QNAP 部署时，应优先使用这个精确 tag 或后续更新的 `latest`，并在浏览器中强刷页面，避免旧站点数据继续显示旧附件 ID。

## 2026-07-09 Android Docker 同步闪退与 NSX 上传预检修复

- 用户反馈：手机 APK 连接今天下午的新 Docker 后会闪退 / 看不到同步数据；Android 端选择 `.nsx` 后预览报 `Failed to fetch`。
- 根因一：Android WebView 从 `capacitor://localhost` 调用 Docker API 时会触发 CORS 预检。NSX 异步上传会带 `X-Async-Import`，但服务端旧 CORS 允许头没有包含它，导致预检失败，前端只能看到 `Failed to fetch`。
- 根因二：连接远程 Docker 成功后，前端仍会尝试把远程大数据快照写入本地离线缓存。如果 Android 本地 IndexedDB / SQLite 写入复杂导入数据失败，旧逻辑会把整个远程加载流程拖进失败分支，表现为看不到同步数据或页面异常。
- 修复：服务端 CORS 允许头新增 `X-Async-Import`；前端读取本地快照、读取待同步队列、写入本地快照全部改为安全兜底。本地缓存失败只记录 warning，不再阻止远程 Docker 数据显示。
- 回归测试：新增 Android NSX CORS 预检测试；新增“Android 本地 snapshot 失败不能隐藏远程 Docker 数据”的静态测试。
- 产物级验证：`npm.cmd run check`、`npm.cmd run test`、`npm.cmd run build`、`npm.cmd run android:build`、`npm.cmd run android:verify` 均通过；本机构建 `note:android-sync-hotfix-test` Docker 镜像并启动 `http://127.0.0.1:3326`，HTTP smoke 通过。
- 真实 `.nsx` 验证：向本地 3326 Docker 上传 22.4MB 真实 `.nsx`，预览结果 93 条 / 成功 93 条 / 失败 0 / 附件 20；确认导入后逐条详情聚合：Note Station 导入记录 94 条，91 条有富文本，5 条有附件列表，5 条有内联附件引用，第一个内联附件文件接口返回 200。
- 给手机测试时，本机新 Docker 地址是 `http://192.168.110.98:3326/`；NAS/GHCR 仍需推送后等待新镜像构建，再让用户拉取新 tag。
- 交付经验：以后 Android + Docker 问题必须同时验证 CORS preflight、真实 Docker 容器 smoke、真实 `.nsx` 上传预览、确认导入后的详情富文本和附件 URL 状态，不能只跑源码单测。

## 2026-07-09 Android 大批量同步闪退修复

- 用户反馈：Android 手机已经可以导入 / 同步 Docker 数据，但 App 会莫名闪退，基本不能正常使用。
- 根因定位：`/api/app-data` 旧逻辑会对首页 / 初始同步列表返回 `includeRichText=true` 的完整记录数据，导致手机一次同步时拉取全部 Note Station 富文本 HTML、source_html、图片引用等大字段；随后 APK 还会尝试把这些远程大字段写入本地 IndexedDB / SQLite 快照，容易给 Android WebView 和本地存储造成内存压力，表现为闪退或页面异常。
- 修复：`/api/app-data` 改为只返回列表所需轻量数据；单条详情仍通过 `/api/notes?id=...` 返回完整富文本和附件信息。
- 修复：Android 本地快照写入前对已同步远程记录剥离 `contentHtml`、`contentJson`、`sourceHtml`、`richContent`、`rawMetadata` 等大字段；本机未同步 / dirty 草稿仍保留富文本，避免离线编辑丢内容。
- 回归测试：`tests/mvp-api.test.js` 锁定 app-data 不携带富文本大字段；`tests/offline-store-behavior.test.js` 锁定已同步远程快照轻量化、本机富文本草稿保留。
- 验证：`npm.cmd run check` 通过，SQLite integrityCheck=ok，当前本地 noteCount=523。
- 验证：`npm.cmd run test` 通过，16 suites / 95 tests / 95 pass。
- 验证：`npm.cmd run build` 通过。
- 验证：`npm.cmd run android:build` / `npm.cmd run android:verify` 通过，APK `nativeShellOnly=false`，大小 25,708,435 bytes。
- Docker 产物验证：本机构建 `note:android-crash-fix-test` 并运行 `http://127.0.0.1:3339`，HTTP smoke 通过；smoke 后 app-data 有 2 条记录且 rich payload 计数为 0；Note Station 详情接口仍有 richContent，inline 附件引用 2 个，附件 2 个。
- 经验：以后 Android 闪退 / 卡死问题不能只测 API 可用；必须检查 app-data 体积、本地快照写入范围、APK 构建产物和 Docker 镜像产物。

### GHCR 精确镜像验证补充

- GitHub Actions `Build Docker image` run `29033964416` 对 commit `f3d73f7d7543aef90c5087e9d241ce4928f34ebe` 构建成功。
- 已拉取并运行 `ghcr.io/ddrcpddr/note:f3d73f7d7543aef90c5087e9d241ce4928f34ebe`，不是本地源码镜像。
- GHCR 容器 `http://127.0.0.1:3340` HTTP smoke 通过，health.build.commit 对应 `f3d73f7d7543aef90c5087e9d241ce4928f34ebe`。
- GHCR smoke 后验证：`/api/app-data` 有 2 条记录且富文本大字段计数 0；Note Station 详情仍有 richContent，inline 附件引用 2 个，附件 2 个。
