...
## 2026-07-07 下一步：原生离线附件后的同步路线

已完成：原生 APK 可为已保存记录添加本机附件 / 图片，并在详情页查看附件列表。

下一步建议按顺序推进：
1. 真机验收本机附件：离线新建记录 -> 详情页添加图片/文件 -> 关闭重开 App -> 附件仍显示。
2. 设计并实现附件同步：恢复联网后将本机附件上传到 Docker/NAS，并把远端附件 ID / 路径回写本机。
3. 原生端 Note Station `.nsx` 导入：重新导入后图片尽量内联到富文本正文，非图片作为附件保留。
4. 补冲突提示：同一记录多手机修改时提示用户选择保留本机或服务器版本。

暂不做：UI 重做、Android 壳路线、复杂发布流程。

## 2026-07-07 下一步：原生附件同步后的路线

当前已经完成：原生 APK 可离线添加本机附件 / 图片，并在恢复连接 Docker/NAS 后把未同步附件随记录创建 / 更新上传到服务端；成功后本机标记为已同步，避免重复上传。UI 继续保持当前家事记原生风格，不做视觉重设计。

建议下一步：
1. 用户真机验收附件同步：断网添加附件 -> 联网同步 -> 浏览器端查看服务端记录附件。
2. 补原生端服务端附件下载 / 打开体验，让第二台手机同步后也能看到附件入口。
3. 再迁入原生端 Note Station `.nsx` 导入，导入后的图片尽量进入富文本正文，非图片作为附件保留。
4. 最后补多设备冲突提示。
