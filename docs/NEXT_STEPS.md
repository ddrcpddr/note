...
4. 确认是否需要一个简单访问口令；默认成员当前只保留“我 / 爱人”。
5. 后续再实现真实附件上传，把新附件保存到 `data/attachments/` 并在数据库中保存元数据。
...
1. 真实附件上传，保存到 `data/attachments/` 并写入附件元数据。
2. 简单访问口令 / PIN。
3. 导入后批量整理未分类记录。
4. 成员改名、头像和颜色编辑。
5. 定时自动备份。
...
已完成：编辑已有记录；删除 / 归档记录。后续继续保持小步提交，每步先测试、再实现、再更新 QA 和项目记忆。
