...
## Gate 7 第一批已完成（2026-06-30）

已完成“分类页 + 设置页”的小步视觉对齐：更接近 V1 的标题层级、卡片留白、图标权重和设置页生活化表达。验证结果：`npm.cmd run check`、`npm.cmd run test`、`npm.cmd run build` 均通过；390px / 430px 分类页和设置页无横向溢出。

下一批建议：

1. Gate 7 第二批：首页 + 搜索页，只调整记录卡标题、摘要、标签、来源和元信息层级。
2. Gate 7 第三批：新建记录 + 详情页，只调整表单节奏、详情标题和底部操作留白。
3. Gate 7 第四批：导入 Note Station + 导航细节，只做留白、步骤条和文案层级微调。

继续保持：不新增功能、不重做 UI、不改数据库、不碰真实导入数据、不生成图片。

## 富文本后续路线（2026-07-01）

当前只完成第一阶段目标：安全只读展示 Note Station 原始 HTML，并保留 `notes.content` 作为搜索、导出和 fallback 主字段。

下一步不要直接进入完整富文本编辑器。若用户确认继续，需要先明确：

1. 编辑后的富文本存储格式：HTML、Markdown 还是其他结构。
2. `notes.content` 是否继续作为纯文本主字段，或变成由富文本生成的搜索字段。
3. 手机端编辑体验：标题、正文、列表、链接、图片、撤销、粘贴的边界。
4. 附件图片如何从 `data/attachments/` 安全映射到正文。
5. JSON / Markdown 导出如何表达富文本和纯文本。
6. 是否允许引入轻量成熟编辑器依赖。

真实试运行优先检查：一条 Note Station 导入记录详情页、原始格式 / 纯文本切换、长 HTML 内容换行、搜索仍按纯文本命中、JSON / Markdown 导出不多出富文本字段。
