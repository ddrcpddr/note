...
1. 按 `docs/ANDROID_WRAPPER_PLAN.md` 完成真实 Android 手机 Web / PWA 试运行。
2. 若试运行稳定，再由用户确认包名、封装路线、最低 Android 版本、NAS 地址配置策略和签名方式后，启动 Android WebView / TWA 工程。

已完成：编辑已有记录；删除 / 归档记录；真实附件上传；简单访问口令 / PIN；导入后批量整理未分类记录；成员改名、头像和颜色编辑；定时自动备份；Markdown 导出；NAS 数据目录读写探测；Android 原生封装前置评估。后续继续保持小步提交，每步先测试、再实现、再更新 QA 和项目记忆。
