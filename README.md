...
- 新建 / 编辑记录时，图片和附件统一从富文本编辑器的“插入”区域进入。
- 旧的正文外独立附件上传入口已移除；详情页附件列表仅用于下载、兼容展示和排查历史 / 导入附件。
- 网页端 Note Station 导入页已支持选择并上传真实 `.nsx` 文件进行 dry-run 预览，确认导入前会自动备份数据库。
- 当前测试数据库可以清空重建；原始 `.nsx` 文件、数据库、附件、备份、导出和日志都属于运行数据，不提交 Git。
